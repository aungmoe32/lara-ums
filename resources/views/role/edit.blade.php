                    ...
                    <form method="POST" action="{{ route('roles.update', $role) }}">
                        ...
                        <div class="mb-6">
                            ...
                            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full"
                                placeholder="Enter your role" :value="old('name', $role->name)" required autofocus />
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            selectAllCheckboxes.forEach(checkbox => {
                const featureId = checkbox.id.split('_')[2];
                const featurePermissions = document.querySelectorAll(`.feature-${featureId}-permission`);
                const allChecked = Array.from(featurePermissions).every(p => p.checked);
                checkbox.checked = allChecked && featurePermissions.length > 0;
            ...
            // Add event listeners to permission checkboxes to update "Select All" state
            document.querySelectorAll('input[name="permissions[]"]').forEach(permissionCheckbox => {
                permissionCheckbox.addEventListener('change', function() {
                    const featureId = this.className.split('-')[1].split('-')[0];
                    const selectAllCheckbox = document.getElementById(`select_all_${featureId}`);
